...
public class DeleteMonitorService {
    ...
    private String getUrl(DeleteMonitorInputs inputs) throws URISyntaxException {
        ...
        urlBuilder.setPath(SITESCOPE_MONITORS_API + DELETE_MONITOR_ENDPOINT);
        if (StringUtils.isNotEmpty(inputs.getFullPathToMonitor())) {
            String fullPathToMonitor = inputs.getFullPathToMonitor().replace(inputs.getDelimiter(), SITE_SCOPE_DELIMITER);
            urlBuilder.addParameter(Inputs.CommonInputs.FULL_PATH_TO_MONITOR, fullPathToMonitor);
        }
