...
public class CommandUtilsTest {
    ...
    public void getWmiComCreateFileTest() throws Exception {
        ...
        assertNotNull(resultStr);
        assertEquals(resultStr, Constants.WMI+ hostname + Constants.USER + username + Constants.PASSWORD + password + Constants.CREATE_FILE_COM + tempFileForResults + Constants.END_FILE_COM);
    }

    @Test
    public void getWmiComExecuteWindComTest() throws Exception {
        String resultStr =  CommandUtils.getWmiComExecuteWindCom(hostname,command,tempFileForResults);
        assertNotNull(resultStr);
        assertEquals(resultStr, Constants.WMI + hostname + Constants.CALL_CMD_COM + command + Constants.WMI_RES_PUT_COM + tempFileForResults + Constants.BACK_SLASH);
    }
}
