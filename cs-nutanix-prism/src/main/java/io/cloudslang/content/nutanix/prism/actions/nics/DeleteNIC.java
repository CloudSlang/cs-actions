...
import static io.cloudslang.content.nutanix.prism.service.NicImpl.deleteNic;
...
import static io.cloudslang.content.nutanix.prism.utils.Inputs.DeleteNICInputs.NIC_MAC_ADDRESS;
...
public class DeleteNIC {
    @Action(name = DELETE_NIC_OPERATION_NAME,
            description = DELETE_NIC_OPERATION_DESC,
            outputs = {
    ...
    public Map<String, String> execute(@Param(value = HOSTNAME, required = true, description = HOSTNAME_DESC) String hostname,
                                       ...
                                       @Param(value = NIC_MAC_ADDRESS, required = true, description = NIC_MAC_ADDRESS_DESC) String nicMacAddress,
        ...
        try {
            final Map<String, String> result = deleteNic(NutanixDeleteNICInputs.builder()
                    .vmUUID(vmUUID)
                    .nicMacAddress(nicMacAddress)
            ...
            if (statusCode >= 200 && statusCode < 300) {
                ...
                results.put(RETURN_RESULT, DELETE_NIC_SUCCESS_DESC);
            } else {
