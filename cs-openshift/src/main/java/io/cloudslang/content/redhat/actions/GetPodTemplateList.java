...
public class GetPodTemplateList {

    @Action(name = GET_POD_TEMPLATE_LIST,
            description = GET_POD_TEMPLATE_LIST_DESC,
            outputs = {
                    //Common outputs
                    ...
                    @Output(value = POD_TEMPLATE_LIST, description = POD_TEMPLATE_LIST_DESC),
                    @Output(value = POD_TEMPLATE_ARRAY, description = POD_TEMPLATE_ARRAY_DESC),
            },
            responses = {
    ...
    public Map<String, String> execute(
        ...
        try {
            ...
            result = new HttpClientGetAction().execute(
                    host + POD_ENDPOINT_1 + namespace + POD_ENDPOINT_3,
                    ANONYMOUS,
                    EMPTY,
                    EMPTY,
                    EMPTY,
            ...
            if (Integer.parseInt(result.get(RETURN_CODE)) != -1) {
                if (Integer.parseInt(result.get(STATUS_CODE)) >= 200 && Integer.parseInt(result.get(STATUS_CODE)) < 300)
                    addPodTemplateListResults(result);
                else {
                    setFailureCustomResults(result, POD_TEMPLATE_LIST, POD_TEMPLATE_ARRAY, DOCUMENT_OUTPUT);
