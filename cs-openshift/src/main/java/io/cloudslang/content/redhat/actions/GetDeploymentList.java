...
public class GetDeploymentList {
    @Action(name = GET_DEPLOYMENT_LIST,
            ...
            outputs = {
                    ...
                    @Output(value = DEPLOYMENT_LIST, description = DEPLOYMENT_LIST_DESC),
                    @Output(value = DEPLOYMENT_ARRAY, description = DEPLOYMENT_ARRAY_DESC),
            },
            responses = {
    ...
    public Map<String, String> execute(
        ...
        try {
            ...
            result = new HttpClientGetAction().execute(
                    host + GET_DEPLOYMENT_STATUS_ENDPOINT_1 + namespace + GET_DEPLOYMENT_STATUS_ENDPOINT_4,
                    ANONYMOUS,
                    EMPTY,
                    EMPTY,
                    EMPTY,
            ...
            if (Integer.parseInt(result.get(RETURN_CODE)) != -1) {
                if (Integer.parseInt(result.get(STATUS_CODE)) >= 200 && Integer.parseInt(result.get(STATUS_CODE)) < 300)
                    addDeploymentListResults(result);
                else {
                    setFailureCustomResults(result, DEPLOYMENT_LIST, DEPLOYMENT_ARRAY, DOCUMENT_OUTPUT);
