...
import static io.cloudslang.content.redhat.utils.Descriptions.GetTemplateList.*;
import static io.cloudslang.content.redhat.utils.Descriptions.GetTemplateList.FAILURE_RETURN_RESULT;
import static io.cloudslang.content.redhat.utils.Descriptions.GetTemplateList.STATUS_CODE_DESC;
import static io.cloudslang.content.redhat.utils.Outputs.OutputNames.*;
import static org.apache.commons.lang3.StringUtils.EMPTY;

public class GetTemplateList {

    @Action(name = GET_TEMPLATE_LIST,
            description = GET_TEMPLATE_LIST_DESC,
            outputs = {
                    //Common outputs
                    ...
                    @Output(value = TEMPLATE_LIST, description = TEMPLATE_LIST_DESC),
                    @Output(value = TEMPLATE_ARRAY, description = TEMPLATE_ARRAY_DESC),
            },
            responses = {
    ...
    public Map<String, String> execute(
        ...
        try {
            ...
            result = new HttpClientGetAction().execute(
                    host + TEMPLATE_ENDPOINT_1 + namespace + TEMPLATE_ENDPOINT_2,
                    ANONYMOUS,
                    EMPTY,
                    EMPTY,
                    EMPTY,
            ...
            if (Integer.parseInt(result.get(RETURN_CODE)) != -1) {
                if (Integer.parseInt(result.get(STATUS_CODE)) >= 200 && Integer.parseInt(result.get(STATUS_CODE)) < 300)
                    addTemplateListResults(result);
                else {
                    setFailureCustomResults(result, TEMPLATE_LIST, TEMPLATE_ARRAY, DOCUMENT_OUTPUT);
