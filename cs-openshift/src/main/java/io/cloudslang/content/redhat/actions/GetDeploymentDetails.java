...
import static io.cloudslang.content.redhat.utils.Descriptions.GetDeplomentList.EXCEPTION_DESC;
import static io.cloudslang.content.redhat.utils.Descriptions.GetDeploymentDetails.*;
import static io.cloudslang.content.redhat.utils.Outputs.OutputNames.*;
import static org.apache.commons.lang3.StringUtils.EMPTY;

public class GetDeploymentDetails {
    @Action(name = GET_DEPLOYMENT_DETAILS,
            description = GET_DEPLOYMENT_DETAILS_DESC,
            outputs = {
                    //Common outputs
        ...
        try {
            ...
            if (Integer.parseInt(result.get(RETURN_CODE)) != -1) {
                if (Integer.parseInt(result.get(STATUS_CODE)) >= 200 && Integer.parseInt(result.get(STATUS_CODE)) < 300)
                    processHttpGetDeploymentDetailsResult(result);
                else {
