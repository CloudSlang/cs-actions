...
import static io.cloudslang.content.rft.utils.InputsValidation.verifyDeleteFileInputs;
import static io.cloudslang.content.utils.OutputUtilities.getFailureResultsMap;
import static org.apache.commons.lang3.StringUtils.EMPTY;
import static org.apache.commons.lang3.StringUtils.defaultIfEmpty;

public class SFTPDeleteFile {
    @Action(name = SFTP_DELETE_FILE,
            description = SFTP_DELETE_FILE_DESCRIPTION,
            outputs = {
    ...
    public Map<String, String> execute(@Param(value = HOST, description = HOST_NAME, required = true) String host,
                                       ...
                                       @Param(value = REMOTE_PATH, description = REMOTE_PATH_DELETE_FILE_DESC, required = true) String remotePath,
                                       @Param(value = REMOTE_FILE, description = REMOTE_FILE_DELETE_DESC, required = true) String remoteFile,
        ...
        final List<String> exceptionMessages = verifyDeleteFileInputs(remoteFile, remotePath, host, port, username, password,
                proxyPort, characterSet, closeSession, connectionTimeout, executionTimeout);
        if (!exceptionMessages.isEmpty()) {
            return getFailureResultsMap(StringUtilities.join(exceptionMessages, NEW_LINE));
        }

        SFTPDeleteFileInputs sftpDeleteFileInputsInputs = SFTPDeleteFileInputs.builder()
