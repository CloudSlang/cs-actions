...
import static io.cloudslang.content.vmware.constants.Constants.RESOURCE_POOL;
import static io.cloudslang.content.vmware.constants.ErrorMessages.COMPUTE_RESOURCE_NOT_FOUND_ON_HOST;
import static io.cloudslang.content.vmware.entities.ManagedObjectType.DATA_CENTER;
import static io.cloudslang.content.vmware.entities.ManagedObjectType.HOST_SYSTEM;
import static io.cloudslang.content.vmware.entities.ManagedObjectType.PARENT;
import static io.cloudslang.content.vmware.entities.ManagedObjectType.VM_FOLDER;
...
import static io.cloudslang.content.vmware.utils.InputUtils.getUrlString;
...
public class ConnectionResources {
    ...
    public ConnectionResources(HttpInputs httpInputs) throws Exception {
        final String connectionContextKey = computeConnectionContextKey(httpInputs.getProtocol(), httpInputs.getHost(),
                httpInputs.getPort(), httpInputs.getUsername());
    ...
    private Connection getVCenterConnection(HttpInputs httpInputs) throws Exception {
        return basicConnection.isConnected() ? basicConnection :
                basicConnection.connect(getUrlString(httpInputs), httpInputs.getUsername(), httpInputs.getPassword(), httpInputs.isTrustEveryone());
    }
    ...
    private void setVmFolderMor() throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ...
        if (dataCenterMor != null) {
            vmFolderMor = (ManagedObjectReference) moRefHandler.entityProps(dataCenterMor, new String[]{VM_FOLDER.getValue()}).get(VM_FOLDER.getValue());
        }
    ...
    private void setResourcePoolMor() throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ...
        if (computeResourceMor != null) {
            resourcePoolMor = (ManagedObjectReference) moRefHandler.entityProps(computeResourceMor, new String[]{RESOURCE_POOL}).get(RESOURCE_POOL);
        }
    ...
    private void setDataCenterMor(VmInputs vmInputs) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ...
        if (isNotBlank(vmInputs.getDataCenterName())) {
            dataCenterMor = getDataCenterMor(vmInputs.getDataCenterName(), morRootFolder, moRefHandler);
        }
        this.setDataCenterMor(dataCenterMor);
    }

    private ManagedObjectReference getDataCenterMor(String dataCenterName, ManagedObjectReference mor, MoRefHandler moRefHandler) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ManagedObjectReference dataCenterMor = moRefHandler.inContainerByType(mor, DATA_CENTER.getValue()).get(dataCenterName);
        if (dataCenterMor == null) {
            throw new RuntimeException(format("Datacenter [%s] not found.", dataCenterName));
        }
    ...
    private ManagedObjectReference getHostMor(String hostname, MoRefHandler moRefHandler, ManagedObjectReference dataCenterMor) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ManagedObjectReference hostMor = moRefHandler.inContainerByType(dataCenterMor, HOST_SYSTEM.getValue()).get(hostname);
        if (hostMor == null) {
            throw new RuntimeException(format("Host [%s] not found.", hostname));
        }
    ...
    private ManagedObjectReference getComputeResourceMor(MoRefHandler moRefHandler, ManagedObjectReference hostMor) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
        ManagedObjectReference computeResourceMor = (ManagedObjectReference) moRefHandler.entityProps(hostMor, new String[]{PARENT.getValue()}).get(PARENT.getValue());
        if (computeResourceMor == null) {
            throw new RuntimeException(COMPUTE_RESOURCE_NOT_FOUND_ON_HOST);
        }
