...
class DeleteSQLInstance {
  ...
  def execute(@Param(value = PROJECT_ID, required = true, encrypted = true, description = PROJECT_ID_DESC) projectId: String,
    ...
    try {
      ...
      SQLOperationStatus(SQLDatabaseInstanceService.delete(httpTransport, jsonFactory, credential, projectId,
        instanceId, async, timeout, pollingIntervalMilli)) match {
