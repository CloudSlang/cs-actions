...
import io.cloudslang.content.google.utils.Constants.SQLInstancesConstant.START_SQL_INSTANCE_OPERATION_NAME
...
import io.cloudslang.content.google.utils.action.Descriptions.SQLDataBaseInstances.START_SQL_INSTANCE_OPERATION_DESCRIPTION
  ...
  def execute(@Param(value = PROJECT_ID, required = true, encrypted = true, description = PROJECT_ID_DESC) projectId: String,
    ...
    try {
      ...
      SQLOperationStatus(SQLDatabaseInstanceService.instanceOperation(httpTransport, jsonFactory, credential, projectId,
        instanceId, START_INSTANCE, async, timeout, pollingIntervalMilli)) match {
