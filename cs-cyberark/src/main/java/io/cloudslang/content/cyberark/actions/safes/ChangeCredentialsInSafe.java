...
import static io.cloudslang.content.cyberark.utils.Constants.ChangeCredentialsInVault.*;
import static io.cloudslang.content.cyberark.utils.Constants.CommonConstants.*;
import static io.cloudslang.content.cyberark.utils.Constants.DeleteAccountConstants.ID;
import static io.cloudslang.content.cyberark.utils.Constants.DeleteAccountConstants.ID_DESCRIPTION;
...
public class ChangeCredentialsInSafe {
    @Action(name = CHANGE_CREDENTIALS_IN_SAFE,
            description = CHANGE_CREDENTIALS_IN_SAFE_DESCRIPTION,
            outputs = {
    ...
    public Map<String, String> execute(
            ...
            @Param(value = ID, description = ID_DESCRIPTION, required = true) String id,
            @Param(value = NEW_CREDENTIALS, description = NEW_CREDENTIALS_DESCRIPTION, required = true) String newCredentials,
        ...
        try {
            ...
            body.put(NEW_CREDENTIALS, newCredentials);

            validateProtocol(protocol);

            Map<String, String> result = new HttpClientPostAction().execute(
                    protocol + PROTOCOL_DELIMITER + hostName + GET_ACCOUNTS_ENDPOINT + FORWARD_SLASH + id + CHANGE_CREDENTIALS_IN_VAULT_ENDPOINT,
                    ANONYMOUS,
                    EMPTY,
                    EMPTY,
                    EMPTY,
