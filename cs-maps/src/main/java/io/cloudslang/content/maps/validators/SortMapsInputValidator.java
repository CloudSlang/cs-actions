...
public class SortMapsInputValidator {

    public ValidationException validate(@NotNull SortMapsInput input) {
        try {
            FieldValidator.validateMap(input.getMap(), input.getMapStart(), input.getMapEnd());
            FieldValidator.validateOrder(input.getSortOrder());
            FieldValidator.validateSortBy(input.getSortBy());
