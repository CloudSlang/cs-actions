...
import io.cloudslang.content.active_directory.services.computers.GetComputerAccountOUService;
...
import static io.cloudslang.content.active_directory.constants.Descriptions.GetComputerAccountOU.*;
...
public class GetComputerAccountOUAction {
     ...
     */
    @Action(name = "Get Computer Account OU", description = GET_COMPUTER_ACCOUNT_OU_DESC,
            outputs = {
                    @Output(value = OutputNames.RETURN_RESULT, description = RETURN_RESULT_DESC),
                    @Output(value = OutputNames.RESULT_OU_DN, description = RESULT_GET_COMPUTER_OU_DESC),
